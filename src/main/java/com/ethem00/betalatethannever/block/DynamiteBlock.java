...
public class DynamiteBlock extends FallingBlock {
    ...
    private static VoxelShape cuboidPxl(float x1, float y1, float z1, float x2, float y2, float z2) {
    ...
    public void onDestroyedByExplosion(World world, BlockPos pos, Explosion explosion) {
        if (!world.isClient) {
            DynamiteEntity dynamiteEntity = new DynamiteEntity(world, pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5, explosion.getCausingEntity());
            int i = dynamiteEntity.getFuse();
            dynamiteEntity.setFuse((short)(world.random.nextInt(i / 4) + i / 8));
            world.spawnEntity(dynamiteEntity);
        }
    }

    public static void primeDynamite(World world, BlockPos pos) {
        primeDynamite(world, pos, null);
    }

    private static void primeDynamite(World world, BlockPos pos, @Nullable LivingEntity igniter) {
        if (!world.isClient) {
            DynamiteEntity dynamiteEntity = new DynamiteEntity(world, pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5, igniter);
            world.spawnEntity(dynamiteEntity);
    ...
    public void onProjectileHit(World world, BlockState state, BlockHitResult hit, ProjectileEntity projectile) {
        if (!world.isClient) {
            ...
            if (projectile.isOnFire() && projectile.canModifyAt(world, blockPos)) {
                primeDynamite(world, blockPos, entity instanceof LivingEntity ? (LivingEntity)entity : null);
